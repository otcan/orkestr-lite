...
signed-out Docker environment. The owner must also approve every screenshot and
the final narrated video. A source-independent workflow failure may be retried.
A source correction requires a new immutable patch version; never move the v0.2
tag.
